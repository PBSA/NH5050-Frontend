...
class Tickets extends Component {
  ...
  render() {
    ...
    /*const proceedSum = {
      total: 'Total',
      empty1: '',
      entries: tickets.reduce((sum, ticket) => sum + ticket.ticket_entries, 0),
      proceeds: tickets.reduce((sum, ticket) => sum + parseFloat(ticket.ticket_value), 0.0).toFixed(2)
    };*/
      ...
      <div>
        <FormControl variant="outlined">
          <InputLabel id="raffle-select">Raffle</InputLabel>
            ...
            >
              {raffles.map((raffle, index) => <MenuItem key={index} value={raffle.id}>Raffle #{raffle.id}</MenuItem>)}
            </Select>
        </FormControl>
